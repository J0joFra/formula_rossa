...
// ─── COORDINATE GPS CIRCUITO (forma reale) ────────────────────────────────────
/**
 * Usa /location per ottenere le coordinate x,y della macchina durante il giro più veloce.
 * Ritorna array di { x, y, speed } per disegnare la mappa reale del circuito.
 */
export async function getCircuitMap(session_key, driver_number) {
  // Trova il giro più veloce
  const laps = await openf1Fetch('/laps', { session_key, driver_number });
  const validLaps = laps.filter(l => l.lap_duration != null && l.lap_duration > 0);
  if (!validLaps.length) return [];

  const fastest = validLaps.reduce((a, b) => a.lap_duration < b.lap_duration ? a : b);
  if (!fastest.date_start) return [];

  const lapStart = new Date(fastest.date_start).getTime();
  const lapEnd = lapStart + (fastest.lap_duration + 2) * 1000;

  // Fetch location (coordinate GPS trasformate in x,y,z)
  const allLocation = await openf1Fetch('/location', { session_key, driver_number });
  if (!allLocation.length) return [];

  // Filtra per il range del giro più veloce
  ...
  // Fetch car_data per matchare la velocità ai punti GPS
  const allCarData = await openf1Fetch('/car_data', { session_key, driver_number });
  ...
  // Crea mappa velocità per data (lookup veloce)
  const speedByTime = new Map();
  lapCarData.forEach(d => speedByTime.set(d.date, d.speed ?? 0));

  // Associa velocità al punto GPS più vicino nel tempo
  return points.map(p => {
    // Trova car_data più vicino per data
    let closest = 0;
    let closestDiff = Infinity;
    for (const [date, speed] of speedByTime) {
      const diff = Math.abs(new Date(date).getTime() - new Date(p.date).getTime());
      if (diff < closestDiff) { closest = speed; closestDiff = diff; }
    }
    return { x: p.x, y: p.y, speed: closest };
...
export async function getRacePositions(session_key) {
  ...
  const byLap = lapNumbers.map(lapNum => {
    ...
    lapsData.filter(l => l.lap_number === lapNum).forEach(lap => {
      ...
      let closest = driverPos[0];
      let closestDiff = Math.abs(new Date(driverPos[0].date).getTime() - lapEndMs);
