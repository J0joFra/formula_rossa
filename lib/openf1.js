...
export async function getLatestSession(session_name = 'Race') {
  const cacheKey = `latest_session_${session_name}`;
  const cached = cache.get(cacheKey);
  if (cached && Date.now() - cached.timestamp < CACHE_TTL) return cached.data;

  try {
    // Prima prova il meeting più recente tramite endpoint meeting_key=latest
    const meetings = await openf1Fetch('/meetings', { meeting_key: 'latest' });
    if (meetings && meetings.length > 0) {
      const meetingKey = meetings[0].meeting_key;
      const sessions = await openf1Fetch('/sessions', { meeting_key: meetingKey, session_name });
    ...
    console.warn('Fallback to manual session search', error);
  }

  // Fallback alla ricerca manuale per anno
  const currentYear = new Date().getFullYear();
  for (const year of [currentYear, currentYear - 1]) {
    try {
      const sessions = await openf1Fetch('/sessions', { year, session_name });
      const now = new Date();
...
// Accetta opzionalmente laps e carData già fetchati per evitare chiamate duplicate
export async function getTelemetry(session_key, driver_number, lap_number = null, prefetchedLaps = null, prefetchedCarData = null) {
  const rawLaps = prefetchedLaps ?? await openf1Fetch('/laps', { session_key, driver_number });
  const validLaps = rawLaps.filter(l => l.lap_duration != null && l.lap_duration > 0);
  ...
  const allCarData = prefetchedCarData ?? await openf1Fetch('/car_data', { session_key, driver_number });
  ...
  return {
    telemetry: points,
    all_laps: validLaps,
    carData: allCarData, // restituito per riuso in refetch/circuitMap
    target_lap: {
...
// Accetta opzionalmente laps e carData già fetchati per evitare chiamate duplicate
export async function getCircuitMap(session_key, driver_number, lap_number = null, prefetchedLaps = null, prefetchedCarData = null) {
  const rawLaps = prefetchedLaps ?? await openf1Fetch('/laps', { session_key, driver_number });
  const validLaps = rawLaps.filter(l => l.lap_duration != null && l.lap_duration > 0);
  ...
  const locationPromise = openf1Fetch('/location', { session_key, driver_number });
  const carDataPromise = prefetchedCarData
    ? Promise.resolve(prefetchedCarData)
    : openf1Fetch('/car_data', { session_key, driver_number });

  const [allLocation, allCarData] = await Promise.all([locationPromise, carDataPromise]);
...
// ─── INTERVALS ────────────────────────────────────────────────────────────────
export async function getIntervals(session_key) {
  return openf1Fetch('/intervals', { session_key });
}

// ─── TEAM RADIO ───────────────────────────────────────────────────────────────
export async function getTeamRadio(session_key, driver_number = null) {
  return openf1Fetch('/team_radio', { session_key, driver_number });
}

// ─── WEATHER ──────────────────────────────────────────────────────────────────
export async function getWeatherData(session_key) {
  return openf1Fetch('/weather', { session_key });
}

// ─── CAR DATA ─────────────────────────────────────────────────────────────────
export async function getCarData(session_key, driver_number) {
  return openf1Fetch('/car_data', { session_key, driver_number });
}
