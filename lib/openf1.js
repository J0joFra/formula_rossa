...
const RATE_LIMIT = 8; // richieste al secondo (leggermente sotto il limite per sicurezza)
const REQUEST_INTERVAL = 1000 / RATE_LIMIT; // ~125ms tra le richieste
...
async function processQueue() {
  ...
  while (requestQueue.length > 0) {
    ...
    try {
      ...
      // Se è 429 e non abbiamo già ritentato troppe volte
      if (error.message.includes('429') && retryCount < 3) {
        console.log(`⚠️ Rate limit hit for ${endpoint}, retrying in ${Math.pow(2, retryCount)}s...`);
        // Rimetti in coda con backoff esponenziale
        setTimeout(() => {
...
export async function getAllDriversSectors(session_key) {
  ...
  // Processa i piloti in batch per non sovraccaricare l'API
  const batchSize = 3;
    ...
    await Promise.allSettled(
      batch.map(async (driver) => {
        try {
          ...
          results.push({
            code: driver.name_acronym, full_name: driver.full_name,
            team: driver.team_name,
            color: driver.team_colour ? `#${driver.team_colour}` : '#ef4444',
            lap_time: fastest.lap_duration, s1: fastest.duration_sector_1,
            s2: fastest.duration_sector_2, s3: fastest.duration_sector_3,
            best_lap_number: fastest.lap_number,
            all_laps: valid.sort((a, b) => a.lap_number - b.lap_number).map(l => ({
              lap_number: l.lap_number, lap_duration: l.lap_duration,
