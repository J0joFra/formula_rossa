/**
 * lib/fantaf1/fantaService.js
 * ===========================
 * Tutte le operazioni Firestore per il FantaF1.
 * Si integra con il tuo firebase.js e tokens.js esistenti.
 *
 * STRUTTURA FIRESTORE:
 *
 * fantaF1Leagues/{leagueId}
 *   ├── name, createdBy, inviteCode, createdAt
 *   ├── members/{userId} → { displayName, avatar, joinedAt }
 *   └── picks/{raceId}/{userId} → { driverNumber, pickedAt }
 *
 * fantaF1Races/{raceId}
 *   ├── name, year, country, date, status ('upcoming'|'completed')
 *   ├── results/{driverNumber} → raceResult (da OpenF1)
 *   ├── manualEvents/{driverNumber} → { EPIC_TEAM_RADIO: bool, ... }
 *   └── scores/{userId} → { driverNumber, autoPoints, manualPoints, total, breakdown }
 */
...
/**
 * L'utente sceglie il suo pilota prima della gara.
 * Può cambiare il pick finché la gara non è "locked".
 */
...
/**
 * Recupera il pick di un utente per una gara.
 */
...
/**
 * Recupera tutti i pick di una lega per una gara (visibili dopo il lock).
 */
...
/**
 * Crea o aggiorna una gara nel calendario FantaF1.
 * Chiamata dall'admin per aggiungere le gare stagionali.
 */
...
/**
 * Recupera tutte le gare stagionali.
 */
...
export async function processRaceScores(raceId, raceResultsMap, manualEventsMap = {}) {
  // Salva i risultati raw su Firestore
  const raceRef = doc(db, 'fantaF1Races', raceId);
  const resultsToSave = {};
  ...
  for (const leagueDoc of leaguesSnap.docs) {
    ...
    // Recupera i pick di tutti i membri per questa gara
    ...
    for (const pickDoc of picksSnap.docs) {
      ...
      // Salva il punteggio della singola gara
      await setDoc(
      ...
      // Aggiorna il punteggio cumulativo del membro nella lega
      await updateDoc(
