...
import { supabase as browser } from './../supabaseClient';
import { db as server } from './../supabaseServer';

/**
 * Il client giusto per il posto in cui gira il codice.
 *
 * Questo modulo serviva solo il browser, quindi importava direttamente il
 * client del browser — che sul server è `null`. Da quando le pagine dei Gran
 * Premi si generano lato server (per farsi leggere da chi indicizza), le
 * stesse funzioni devono funzionare da entrambe le parti. La chiave è la
 * stessa anon in tutti e due i casi: cambia solo dove viene costruita.
 */
const supa = () => (typeof window !== 'undefined' ? browser : server());
