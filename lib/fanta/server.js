...
export async function salvaPrevisione({ utente, year, round, previsione }) {
  ...
  /* Si gioca una gara alla volta, quella in corso. La pagina non offre altro,
     ma la pagina è il browser: senza questo controllo basta una chiamata a
     mano per riempire in anticipo tutte le domeniche della stagione. */
  const corrente = await prossimaGara(year);
  if (corrente.round !== round) {
    throw new ErroreFanta(409, 'Si gioca solo il Gran Premio in corso.');
  }
