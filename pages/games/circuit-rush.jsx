...
import { ChevronLeft, Zap, AlertTriangle, RefreshCw, Gauge } from 'lucide-react';
import Navigation from '../../components/ferrari/Navigation';
import Footer from '../../components/ferrari/Footer';
import { useSession } from "next-auth/react";
import { addTokens, getTokens, initUser } from '../../lib/tokens'; // ← Firestore helper
...
export default function CircuitRush() {
  ...
  const [totalTokens, setTotalTokens] = useState(0); // ← Tokens letti da Firestore
  ...
  useEffect(() => {
    if (!session) return;
    const init = async () => {
      await initUser(session);           // crea il doc se è il primo login
      const t = await getTokens(session); // legge i token attuali
      setTotalTokens(t);
    };
    init();
  }, [session]);

  // Salva token su Firestore a fine partita
  const saveTokens = async (points) => {
    if (!session) return;
    const tokensEarned = Math.floor(points / 5);
    setEarnedTokens(tokensEarned);
    await addTokens(session, tokensEarned);         // scrive su Firestore
    setTotalTokens(prev => prev + tokensEarned);    // aggiorna UI locale
  };
        ...
        <div className="relative w-full max-w-[400px] h-[650px] bg-zinc-950 rounded-[40px] border-4 border-zinc-800 overflow-hidden shadow-[0_0_50px_rgba(255,0,0,0.1)]">
          ...
          {gameState === 'playing' && (
            <div className="absolute inset-0 z-40 flex">
              <div className="w-1/2 h-full active:bg-white/5 transition-colors" onPointerDown={moveLeft} />
              <div className="w-1/2 h-full active:bg-white/5 transition-colors" onPointerDown={moveRight} />
            </div>
          )}
          ...
          <AnimatePresence>
            {gameState === 'idle' && (
              <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="absolute inset-0 z-50 bg-black/80 flex flex-col items-center justify-center p-8 text-center backdrop-blur-md">
                ...
                <p className="text-zinc-400 text-sm mb-2">Tocca i lati dello schermo per sterzare!</p>
                {session && (
                  <p className="text-yellow-500 text-xs font-black mb-8">
                    Bilancio attuale: {totalTokens.toLocaleString()} SFT
                  </p>
                )}
                <button onClick={startGame} className="w-full py-4 bg-red-600 text-white rounded-2xl font-black uppercase tracking-widest shadow-lg">
                  Start Engine
                </button>
              </motion.div>
            )}
            ...
            {gameState === 'gameover' && (
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="absolute inset-0 z-50 bg-black/95 flex flex-col items-center justify-center p-8 text-center backdrop-blur-md">
                ...
                <div className="bg-yellow-500/10 border border-yellow-500/20 p-6 rounded-3xl w-full mb-3">
                ...
                {/* Totale aggiornato */}
                <p className="text-zinc-500 text-xs font-bold mb-8">
                  Totale: <span className="text-white">{totalTokens.toLocaleString()} SFT</span>
                </p>
          ...
          <div className="absolute inset-0 pointer-events-none opacity-30">
            {[...Array(6)].map((_, i) => (
              <motion.div key={i} initial={{ y: -100 }} animate={{ y: 750 }}
                transition={{ duration: 2.5 / (gameSpeed / 5), repeat: Infinity, ease: "linear", delay: i * 0.4 }}
