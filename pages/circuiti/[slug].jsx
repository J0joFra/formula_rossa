    ...
    <>
      <Head>
        ...
        <meta name="description"
          content={`${circuit.name}: lunghezza ${circuit.lap_length_km ?? '?'} km, ${circuit.number_of_corners ?? '?'} curve, ${circuit.number_of_drs_zones ?? '?'} zone DRS.`}
        />
      </Head>
      <div className="min-h-screen bg-zinc-950 text-white">
        <Navigation />
        <main className="max-w-5xl mx-auto px-4 pt-24 pb-20">
          ...
          <div className="relative overflow-hidden rounded-2xl border border-white/8 mb-8"
            ...
            <div className="relative z-10 p-8 md:p-12">
                ...
                <div>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className="text-[11px] text-white/40 font-mono tracking-[0.25em] uppercase">
                      ...
                      {circuit.city_name ? ` · ${circuit.city_name}` : ''}
                    </span>
                  </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mt-8">
                <HeroStat label="Lunghezza"     value={circuit.lap_length_km ? `${circuit.lap_length_km} km` : '—'} accent />
                <HeroStat label="Curve"         value={circuit.number_of_corners ?? '—'} />
                <HeroStat label="Zone DRS"      value={circuit.number_of_drs_zones ?? '—'} />
                <HeroStat label="Prima gara F1" value={circuit.season_debut ?? '—'} />
              </div>
            </div>
          </div>
            ...
            <div className="space-y-6">
              <div className="rounded-2xl border border-white/8 p-6"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  {[
                    { label: 'Lunghezza giro',  value: circuit.lap_length_km ? `${circuit.lap_length_km} km` : '—' },
                    { label: 'Numero di curve', value: circuit.number_of_corners ?? '—' },
                    { label: 'Zone DRS',        value: circuit.number_of_drs_zones ?? '—' },
                    { label: 'Direzione pista', value: circuit.direction ?? '—' },
                    { label: 'Prima gara F1',   value: circuit.season_debut ?? '—' },
                    { label: 'Città',           value: circuit.city_name ?? '—' },
          ...
          {/* Tab: Storia gare — placeholder, races not yet on Supabase */}
          {tab === 'history' && (
            <div className="rounded-2xl border border-white/8 p-8 text-center"
              ...
              <p className="text-white/15 font-mono text-xs mt-2">
                Migrare la tabella races su Supabase per abilitare questa sezione
              </p>
            </div>
          )}
