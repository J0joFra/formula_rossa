...
function QualifyingToRaceProgression({ raceResults, year, grandPrix }) {
  ...
  if (!drivers.length) return (
    <div className="relative overflow-hidden rounded-2xl border border-white/8 p-10 text-center"
      style={{background:'linear-gradient(135deg,#0d0d0d,#111111)'}}>
      <p className="text-zinc-600 font-mono text-sm tracking-widest uppercase">No race results available</p>
    </div>
  );
  ...
  const LX = 230, RX = 730;   // vertical rail X positions
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/8"
      ...
      {/* ── Top scan line (animated) ── */}
      <div className="absolute top-0 left-0 right-0 h-px pointer-events-none overflow-hidden">
        <div className="h-px w-1/3 bg-gradient-to-r from-transparent via-red-500/70 to-transparent animate-[scan_4s_linear_infinite]"
      ...
      <div className="relative z-10 p-6 md:p-10">
        ...
        <div className="flex flex-wrap items-start justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-zinc-500 font-mono text-xs mt-2 tracking-[0.2em] uppercase">
              {year} · {grandPrix} · Position Gains & Losses
            </p>
          </div>
        ...
        <div className="overflow-x-auto rounded-xl" style={{background:'rgba(0,0,0,0.35)',border:'1px solid rgba(255,255,255,0.05)'}}>
          <svg width="100%" viewBox={`0 0 ${SVG_W} ${SVG_H}`} style={{minWidth:500,display:'block'}}>
            ...
            {/* ── Column labels ── */}
            <text x={LX/2 + 15} y={28} textAnchor="middle" fill="#3f3f46"
              fontSize={10} fontFamily="monospace" letterSpacing="6">GRIGLIA PARTENZA</text>
            <text x={(SVG_W + RX)/2 - 15} y={28} textAnchor="middle" fill="#3f3f46"
              fontSize={10} fontFamily="monospace" letterSpacing="6">ARRIVO GARA</text>

            {/* ── Vertical rails ── */}
            <line x1={LX} y1={36} x2={LX} y2={SVG_H - 8}
              stroke="rgba(255,255,255,0.12)" strokeWidth={1} strokeDasharray="3 6"/>
            <line x1={RX} y1={36} x2={RX} y2={SVG_H - 8}
              stroke="rgba(255,255,255,0.12)" strokeWidth={1} strokeDasharray="3 6"/>

            {/* ── Zebra row bands ── */}
            {byGrid.map((d, i) => i % 2 === 0 && (
              <rect key={`z-${d.id}`} x={LX+1} y={gridY[d.id] - ROW_H/2}
                width={RX - LX - 2} height={ROW_H}
                fill="rgba(255,255,255,0.014)" />
            ))}

            {/* ── CURVES: dimmed pass first, highlighted on top ── */}
              ...
              filtered.map(d => {
                ...
                return (
                  <g key={d.id}>
                    ...
                    {/* Outer glow on hover */}
                    {isHL && <path d={path} fill="none" stroke={d.color} strokeWidth={16}
                      opacity={0.15} strokeLinecap="round" />}

                    {/* Main line */}
                    <path d={path} fill="none"
                      stroke={`url(#g-${d.id})`}
                      strokeWidth={sw} opacity={opacity} strokeLinecap="round" />

                    {/* Endpoint halos on hover */}
                    {isHL && <>
            ...
            {byGrid.map(d => {
              ...
              return (
                <g key={`L-${d.id}`}
                  ...
                  {isHL && <rect x={6} y={y - 14} width={LX - 16} height={28}
                    rx={5} fill={d.color} fillOpacity={0.12} />}

                  {/* Position number */}
                  <text x={36} y={y + 5} textAnchor="middle"
                    fontSize={dim ? 9 : isHL ? 11 : 10}
            ...
            {byRace.map(d => {
              ...
              return (
                <g key={`R-${d.id}`}
                  ...
                  {isHL && <rect x={RX + 16} y={y - 14} width={SVG_W - RX - 22} height={28}
                    rx={5} fill={d.color} fillOpacity={0.12} />}
                  ...
                  {/* Position + delta */}
                  <text x={SVG_W - 10} y={y + 5} textAnchor="end"
                    fontSize={dim ? 9 : isHL ? 11 : 10}
                    fontWeight={700} fontFamily="monospace"
            ...
            {highlight && (() => {
              ...
              return (
                <g>
                  {/* Card shadow */}
                  <rect x={midX - tw/2 + 3} y={midY - 42 + 3} width={tw} height={78}
                    rx={10} fill="rgba(0,0,0,0.6)" />
                  {/* Card body */}
                  <rect x={midX - tw/2} y={midY - 42} width={tw} height={78}
                    rx={10} fill="#0d0d0d" stroke={d.color} strokeWidth={1.5} strokeOpacity={0.7} />
                  {/* Left accent */}
                  <rect x={midX - tw/2} y={midY - 42} width={4} height={78} rx={3} fill={d.color} />
                  {/* Team color dot */}
                  <circle cx={midX - tw/2 + 20} cy={midY - 18} r={4} fill={d.color} />
                  {/* Code */}
                  <text x={midX - tw/2 + 32} y={midY - 14} textAnchor="start"
                    fill="#ffffff" fontSize={16} fontWeight={900} fontFamily="monospace">{d.code}</text>
                  {/* Route */}
                  <text x={midX - tw/2 + 12} y={midY + 2} textAnchor="start"
                    fill="#6b7280" fontSize={9.5} fontFamily="monospace" letterSpacing={1}>
                    GRID P{d.gridPos}  →  RACE P{d.racePos}
                  </text>
                  {/* Delta */}
                  <text x={midX} y={midY + 22} textAnchor="middle"
                    fill={gc} fontSize={14} fontWeight={900} fontFamily="monospace">
                    {d.gain > 0 ? `▲ +${d.gain} POSITIONS` : d.gain < 0 ? `▼ ${d.gain} POSITIONS` : '● NO CHANGE'}
                  </text>
                  {d.points > 0 && (
                    <text x={midX} y={midY + 36} textAnchor="middle"
                      fill="#3f3f46" fontSize={9} fontFamily="monospace">{d.points} PTS</text>
                  )}
                </g>
              );
            })()}
        ...
        <div className="flex flex-wrap items-center gap-6 mt-5 pt-5 border-t border-white/5">
          ...
          {[
            {color:'#22c55e', label:'Gained positions'},
            {color:'#ef4444', label:'Lost positions'},
            {color:'#71717a', label:'No change'},
          ].map(({color,label}) => (
            <span key={label} className="flex items-center gap-2 text-[11px] text-zinc-500 font-mono">
              <span className="inline-block w-8 h-0.5 rounded" style={{background:color}} />
              {label}
            </span>
          ))}
