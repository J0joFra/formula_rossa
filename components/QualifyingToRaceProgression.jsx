...
function GridToRaceChart({ raceResults, year, grandPrix }) {
  ...
  if (!drivers.length) return (
    <div className="flex items-center justify-center h-40 text-[var(--fr-text-faint)] font-mono text-sm tracking-widest uppercase">
  ...
  const FILTERS = [
    ...
    { key: 'same',   label: `● SAME · ${stats.same}`,     ac: 'bg-[var(--fr-surface-2)]/60 text-[var(--fr-text-muted)] border-[var(--fr-border)]/40' },
  ];
  ...
  return (
    <div className="space-y-6">
        ...
        {FILTERS.map(({ key, label, ac }) => (
          <button key={key} onClick={() => setFilter(key)}
            className={`px-3.5 py-2 rounded-xl text-[11px] font-mono tracking-widest border transition-all duration-200 ${
              filter === key ? ac : 'bg-white/5 text-[var(--fr-text-faint)] border-white/8 hover:text-[var(--fr-text-muted)] hover:bg-white/8'
            }`}>
            {label}
          </button>
        ))}
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          <div key={label} className="rounded-xl px-4 py-3" style={{ background: bg, border: `1px solid ${border}` }}>
            <div className="text-[9px] text-[var(--fr-text-faint)] font-mono tracking-[0.25em] uppercase mb-1">{label}</div>
      ...
      <div className="flex flex-wrap items-center gap-6 pt-4 border-t border-white/5">
        <span className="text-[9px] text-[var(--fr-text-faint)] font-mono tracking-[0.3em] uppercase">Legend</span>
        <span className="flex items-center gap-2 text-[11px] text-[var(--fr-text-muted)] font-mono">
          <span className="inline-block w-8 h-0.5 rounded" style={{ background: 'linear-gradient(90deg,#888,#888)' }} /> Team color (all lines)
        </span>
        <span className="ml-auto text-[10px] text-[var(--fr-text-dim)] font-mono tracking-widest">HOVER TO HIGHLIGHT</span>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-6 pt-4 border-t border-white/5">
        ...
        {[
          ...
          <span key={label} className="flex items-center gap-2 text-[11px] text-[var(--fr-text-muted)] font-mono">
      ...
      <div className="relative z-10 p-6 md:p-10">
        ...
        <div className="flex flex-wrap items-start justify-between gap-6 mb-8">
          <div>
            ...
            <p className="text-[var(--fr-text-muted)] font-mono text-xs mt-2 tracking-[0.2em] uppercase">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-[11px] font-mono tracking-widest border transition-all duration-200 ${
                mode === 'race'
                  ...
                  : 'bg-white/5 text-[var(--fr-text-faint)] border-white/8 hover:text-[var(--fr-text-muted)] hover:bg-white/8'
              }`}>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-[11px] font-mono tracking-widest border transition-all duration-200 ${
                mode === 'quali'
                  ...
                  : hasQuali
                    ? 'bg-white/5 text-[var(--fr-text-faint)] border-white/8 hover:text-[var(--fr-text-muted)] hover:bg-white/8'
                    : 'bg-white/3 text-[var(--fr-text-dim)] border-white/5 cursor-not-allowed opacity-40'
              }`}>
