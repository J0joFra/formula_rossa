...
import { Home as HomeIcon, BarChart3, Gamepad2, LogOut, Gauge, Trophy, Zap, Info } from 'lucide-react';
              ...
              <div className="pt-6 border-t border-white/5">
                {session ? (
                  <div className="flex items-center justify-between bg-white/5 p-4 rounded-2xl">
                    ...
                    <button onClick={() => signOut()} className="p-2 bg-red-600/10 text-red-500 rounded-lg" aria-label="Logout">
...
function NavLink({ href, label, icon: Icon, active }) {
  return (
    ...
    >
      ...
      {label}
      {/* Indicatore attivo — linea rossa sotto */}
      {active && (
        <motion.div
    ...
    <Link
      ...
      className={`flex items-center justify-between w-full px-4 py-3 rounded-xl font-black uppercase text-xs tracking-widest transition-all
        ${active
          ...
          : 'text-zinc-400 hover:text-white hover:bg-red-600/10'
        }`}
      onClick={onClick}
    >
      {label}
